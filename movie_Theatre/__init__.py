...
import movie_Theatre.customer
import movie_Theatre.genre
import movie_Theatre.manager
import movie_Theatre.movie
import movie_Theatre.showing
import movie_Theatre.theatreRoom
...
@app.route('/customerMenu', methods=["POST"])
def customerMenu():
    cnx = mysql.connector.connect(user='root', database='MovieTheatre')
    cursor = cnx.cursor()
    ...
    if FirstName != "' or 1=1 #" or LastName != "' or 1=1 #" or EmailAddress != "' or 1=1 #" :
        ...
        cnx.commit()
        cnx.close()
        return render_template('customermenu.html', FirstName=request.form['FirstName'], LastName=request.form['LastName'], EmailAddress=request.form['EmailAddress'], Sex=Sex)
    else:
...
def returnStartMenu():
    return render_template('customermenu.html', FirstName=session.get('FirstName', None), LastName=session.get('LastName', None), EmailAddress=session.get('EmailAddress', None), Sex=session.get('Sex', None))	
